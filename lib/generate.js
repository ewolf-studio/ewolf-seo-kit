...
function srOnlyTag(cfg) {
  ...
  // Style inline de masquage accessible : le kit ne dépend pas du CSS du site
  // (la classe .sr-only reste posée pour ceux qui la stylisent aussi).
  const HIDE = 'position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0';
  return `<p class="sr-only" style="${HIDE}">${escHtml(full)}</p>`;
}
